...
#     salts["autosalSAMPNO"] = salts["autosalSAMPNO"].astype(
#         object
#     )  #   Change dtype for integers and strings
...
#     # ugly hack to work around the ugly hack i made in _salt_exporter (AS)
#     saltDF = pd.DataFrame(data={"cast_id": 1}, index=np.arange(5))
#     # saltDF["CRavg"] = np.ones(5)
#     saltDF['btl_fire_num'] = [n for n in range(5)]
#     saltDF['SALNTY'] = [0 for n in range(5)]
#     saltDF["CRavg"] = np.ones(5)
#     saltDF['BathTEMP'] = [0 for n in range(5)]
#
#     # check test data completes round trip
#     assert not Path(tmp_path / "00101_salts.csv").exists()
#     _salt_exporter(saltDF, btldir=None, outdir=str(tmp_path))
