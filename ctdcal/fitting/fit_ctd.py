...
from ctdcal.common import validate_dir, get_ssscc_list
...
def calibrate_temperature(btl_df, time_df, fit_groups, fit_coeffs, outdir, report_dir, cast_id='SSSCC'):
    """
...
def calibrate_conductivity(btl_df, time_df, fit_groups, fit_coeffs, outdir, report_dir, flag_file, cast_id_col='SSSCC', ref_node='salt'):
    """
    ...
    time_df : DataFrame
        Continuous CTD data
    fit_groups : dict
        Groups of cast ids for fitting
    fit_coeffs : dict
        Fitting parameters for each fit group
    outdir : str or Path-like
        Path to the output directory
    report_dir : str or Path-like
        Path to the report directory
    flag_file : str or Path-like
        Path to the manual flag file
    cast_id_col : str
        Name of cast ID column
    ref_node : str
        Name of parameter to flag
    ...
    if salt_flags_manual is not None:
        ...
        salt_flags_manual_df = salt_flags_manual_df.rename(
                columns={"cast_id": cast_id_col, "bottle_num": "btl_fire_num", "value": "SALNTY_FLAG_W"},
        ).drop(columns=["notes"])
        btl_df = btl_df.merge(salt_flags_manual_df, on=[cast_id_col, "btl_fire_num"], how="left")
    ...
    for sensor_idx, sensor in sorted(enumerate(fit_coeffs)):  # ['primary', 'secondary'] because they are alphabetically in the order we want
        ...
        for coeffs_idx, coeffs in enumerate(fit_coeffs[sensor]):
            casts = fit_groups[coeffs_idx]
            btl_rows = btl_df[cast_id_col].isin(casts).values
            good_rows = btl_rows & (btl_df['SALNTY_FLAG_W'] == 2)
            time_rows = time_df[cast_id_col].isin(casts).values
            ...
            _intermediate_residual_plot(
                ...
                btl_df.loc[btl_rows, 'CTDPRS'],
                btl_df.loc[btl_rows, cast_id_col],
