...
* `ctdcal` now has a `--debug` flag to display all logging levels (instead of WARNING and above)
* `ctdcal quick-convert` command line option to create Exchange ct1 files from Sea-Bird .cnv files, without performing any calibration
* `ctdcal qc` command line option to launch interactive data flagging tool
